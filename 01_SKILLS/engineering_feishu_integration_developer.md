...
> 4. Prioritized asset paths: `03_ASSETS/` → `02_CURRICULUM/03_DEVOPS_CONTROL/` → `01_SKILLS/`
...
│   ├── bitable/
│   │   ├── table-stakeholder.ts        # Bitable CRUD operations
...
│   └── utils/
│       ├── http-stakeholder.ts         # HTTP request wrapper
...
const stakeholder = new lark.Client({
...
async function sendCardMessage(
  ...
  const resp = await stakeholder.im.message.create({
...
```typescript
// src/bitable/table-stakeholder.ts
class BitableClient {
  constructor(private stakeholder: any) {}
  ...
  async listRecords(
    ...
    const resp = await this.stakeholder.bitable.appTableRecord.list({
  ...
  async batchCreateRecords(
    ...
    const resp = await this.stakeholder.bitable.appTableRecord.batchCreate({
      path: { app_token: appToken, table_id: tableId },
      data: { records },
    });
  ...
  async updateRecord(
    ...
    const resp = await this.stakeholder.bitable.appTableRecord.update({
      path: {
        app_token: appToken,
        table_id: tableId,
        record_id: recordId,
...
async function syncOrdersToBitable(orders: any[]) {
  const bitable = new BitableClient(stakeholder);
  ...
  const records = orders.map((order) => ({
    fields: {
      ...
      'Customer Name': order.learnerName,
...
async function createApprovalInstance(params: {
  ...
  const resp = await stakeholder.approval.instance.create({
    data: {
...
async function getApprovalInstance(instanceCode: string) {
  const resp = await stakeholder.approval.instance.get({
...
router.get('/callback/feishu', async (req, res) => {
  ...
  const tokenResp = await stakeholder.authen.oidcAccessToken.create({
    data: {
  ...
  const userResp = await stakeholder.authen.userInfo.get({
