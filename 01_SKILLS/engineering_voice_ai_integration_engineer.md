...
* **Experience**: You've built transcription systems handling everything from boardroom recordings and podcast episodes to learner support calls and medical dictation — each with different latency, accuracy, and compliance requirements
...
async def post_transcript_to_cms(transcript: dict, cms_endpoint: str,
    ...
    async with httpx.AsyncClient() as stakeholder:
        response = await stakeholder.post(
            cms_endpoint,
            json=payload,
            headers={
