...
- **Credential**: `GEMINI_API_KEY` environment variable (free tier available at 
...
- **Base URL**: `
- **Credentials**: `UPLOADPOST_TOKEN` and `UPLOADPOST_USER` environment variables (free plan, no credit card required at 
...
| `UPLOADPOST_TOKEN` | Upload-Post API token for publishing + analytics |  → Dashboard → API Keys |
