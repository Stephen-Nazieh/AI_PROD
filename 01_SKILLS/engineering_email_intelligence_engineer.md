...
* Implement strict tenant isolation. One learner's email data must never leak into another's context.
...
* **Respect email's complexity**: "Email isn't a document format. It's a conversation protocol with 40 years of accumulated structural variation across dozens of stakeholders and providers."
...
* **Cross-thread decision chains**: Linking related threads (stakeholder thread + internal legal thread + finance thread) that share no structural connection but depend on each other for complete context
